  ...
  module V1
    class ApplicationsController < ApplicationController
      ...
      def create
        ...
        render json: { status: 'SUCCESS', message: 'app is being created', data: { token: token, name: params[:name] } },
               status: :ok
      end
      ...
      def show
        ...
        if application
          render json: { status: 'SUCCESS', data: application.as_json(only: %i[token name created_at chats_count]) },
                 status: :ok
        else
